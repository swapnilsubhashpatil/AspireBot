    ...
    if (emailAddress && password) {
      try {
        const response = await axios.post(
          "https://aspirebot.onrender.com/loginUser",
          {
            emailAddress,
            password,
          }
        ...
        if (error.response?.status === 401) {
          toast.warn(
            "User not found or invalid credentials! Please check your details.",
            {
              position: "top-right",
              autoClose: 5000,
              hideProgressBar: false,
      ...
      <div className={Style.mainDiv}>
        <div className={Style.loginDivMain}>
          <div className={Style.NavBarLoginDiv}>
            <h1>AspireBot</h1>
            <Link className={Style.createAccountLink} to="/createAccount">
              Create Account
            </Link>
          </div>
          <div className={Style.loginDiv1}>
            <div className={Style.loginContentDiv}>
              <h1 className={Style.loginContentDivHeading}>
                Sign in to AspireBot
              </h1>
              <p className={Style.loginContentDivPara}>
                Discover. Learn. Achieve. Join CareerClimb Today
              </p>
